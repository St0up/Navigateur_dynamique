...
class Arete:
    ...
    def __init__(self,info_arete):
        ...
        """
        self.nom = (info_arete[0], info_arete[1])    # sommet de départ et d'arrivée
    ...
    def calcul_vitesse(self, nb_car):
        ...
        renvoie La vitesse sur la route selon le nombre de voitures, et la limitation de  vitesse imposé sur celle-ci
        """
        if nb_car == 0 : 
            return self.vitesse_max                     # Si la route est vide la nouvelle voiture est à la vitesse limite      
        if nb_car/(self.longueur/1000) > 142:           # On vérifie que la limite de 142 voitures par kilomètre est respecté
            return 0
        elif self.vitesse_max == 8.3:                   #Sinon cas par cas, selon les vitesses

...
def constru_rep(ind, pere):
    ...
    l.append((pere[ind][1],(pere[ind][0],ind)))
    return l
       
def dijkstra_dynamique(initial, arrivee, graphe, t):
    """
    ...
    temps_traj[initial] = t                                    # conditions initiales
    pere[initial][1] = t
    for loop in range(n):
        ...
        if ind == arrivee:                                     # si le sommet ind = arrivee alors on a le chemin le plus court entre initial et arrivee, donc on arêtte le programme
            return (temps_traj[ind],constru_rep(ind, pere))
        adjoint = adjacent(graphe, ind, do)                      # regarde les arêtes à étudier
        for i in adjoint:
            duree = (graphe.arete)[i].temps(temps_traj[ind])   # duree entre les deux sommets, faire attention à faire la différence entre la fonction temps et la liste
